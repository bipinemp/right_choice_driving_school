...
const Packages = () => {
  return (
    <section className="flex flex-col gap-10 items-center my-20">
      <h1 className="font-black opacity-80">Test Packages</h1>
      <div className="grid grid-cols-2 justify-center items-center gap-x-20">
        {PackagesList.map((pack, index) => (
          <div
            key={index}
            className="bg-gradient-to-r from-primary/50 to-primary w-[350px] text-background h-[450px] py-6 rounded-lg border border-input shadow flex flex-col items-center justify-between"
          >
            ...
            <div className="w-full flex flex-col">
              {pack.desc.map((desc, index) => (
                <p
                  key={index}
                  className={cn(
                    "font-semibold text-center py-2 text-sm opacity-90",
                    {
